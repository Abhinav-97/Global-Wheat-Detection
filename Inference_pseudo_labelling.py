...
parser.add_argument('--test_dir', type=str, default='test', help='train directory path')
...
parser.add_argument('--image_size', type=int, default=args["image_size"], help='image Size')
...
def run_wbf(predictions, image_index, image_size=args["image_size"], iou_thr=0.44, skip_box_thr=0.43, weights=None):
    ...
    for i, image in enumerate(images):
        ...
        for box in boxes:
            ...
            result = {
                ...
                'source': 'nvnn',
                'x': box[0].astype(np.float),
                'y': box[1].astype(np.float),
                'w': box[2].astype(np.float),
                'h': box[3].astype(np.float)
            }
            results.append(result)

test_df_pseudo = pd.DataFrame(results, columns=['image_id', 'width', 'height', 'source', 'x', 'y', 'w', 'h'])
test_df_pseudo.head()
...
fold_number = 0
test_df_pseudo_grouped = test_df_pseudo.groupby('image_id').count()
test_df_pseudo_grouped.drop(["x", "y", "w", "h", "width", "height"], axis=1, inplace=True)
test_df_pseudo_grouped["bbox_count"] = 0
test_df_pseudo_grouped["source"] = 'nvnn'
test_df_pseudo_grouped["stratify_group"] = 'nvnn'
test_df_pseudo_grouped["fold"] = fold_number + 1

df_folds = pd.concat([df_folds, test_df_pseudo_grouped])

frames = [marking, test_df_pseudo]

marking = pd.concat(frames)
...
def get_train_transforms():
    return A.Compose(
        [
            ...
            A.RandomBrightnessContrast(brightness_limit=0.3, contrast_limit=0.3, brightness_by_max=True, p=1.0),
...
class DatasetRetriever(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        if self.test or random_num > 0.5:
            ...
            # if random_num < 0.325:
            #    image, boxes = self.load_cutmix_2(index)
            # else:
    ...
    def load_image_and_boxes(self, index):
        ...
        if 'nvnn' in image_id:
            ...
            image = cv2.imread(f'{DIR_TEST}/{new_image_id}.jpg', cv2.IMREAD_COLOR)
        else:
    ...
    def fit(self, train_loader, validation_loader):
        for e in range(self.epoch, self.config.n_epochs):
            ...
            if summary_loss.avg < self.best_summary_loss:
                ...
                self.model.eval()
                self.save(f'{self.base_dir}/best-checkpoint-psuedolabelling.bin')
...
IS_TEST = len(os.listdir(DIR_TEST)) > 11
...
results = []
if IS_TEST:
    net = load_net('best-checkpoint-psuedolabelling.bin')
else:
    net = load_net(args["model_path"])
for images, image_ids in data_loader:
    predictions = make_tta_predictions(images)
